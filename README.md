...
结局（门后世界打开）才会播放 —— 对应 spec §11「完成最终任务后」。

但**结局只看「当天有没有行动」，不看总能量**（`finaleEarned`）。这里曾经写错过：
开门额外要求 180 总能量（= 6 天满勤），于是**每天只完成 1 个目标的孩子走到 Day 7
什么都看不到** —— D6 承诺的门永远不开，首页还对他说「这段旅程到这里就完整了」。
那正好和 P5（不完成也不惩罚）相反。
...
| | 判定依据 | 做得少的孩子看到 |
| --- | --- | --- |
| 宠物 / 植物 | `min(能量阶梯, 当天上限)` | **真实的**自己那棵树、那只狐狸（不能替他长） |
| 门后世界（结局） | 当天有行动 | 一定打开 |

奖励文案里「门后的世界打开了！」优先级最高（在开花之前），否则跨过 180 的那一下
会被植物的"长出新叶子"抢走，门在背后悄悄打开而孩子根本没被告知。
...
孩子每两三天才跨过一个阈值。实测（`tests/domain/reward.test.ts` 钉住）改前的规则：
...
那几天出现的花、蝴蝶、门是**日历**给的 —— 孩子什么都没做也会出现。而
「孩子回来是因为想看**自己的**世界变成什么样」这句话，恰恰在这些日子里没有落点。

所以世界现在有第二条轴：**`starsEarned` = 当天有行动的天数**，画成岛上的
「小星星园」（一天一颗，7 个位置，空的画成虚线）。

- 每天第一个目标奖励「小星星园亮起了一颗新的星星」
- 同一天后续目标不再长星（那一天仍有其他反馈 beat）
- **不参与任何内容解锁、不影响 Day Gate** —— 纯粹是「我来了」的可见记录
- 从 `goalsByDay` 派生，不额外存储；reset 自动清零
- 做得少的孩子因此**每天都能看到属于自己的变化**，做得多的孩子也不会少什么

> 顺带补上了评估文档 §4 指出的结构缺口：改前 D1–D6 的大变化**全部**由日历触发，
> 所以「行为驱动世界变化」这件事在 Phase 0 里根本没有被测量过。
...
```text
Background → Sky → Sun → Clouds → Ground → NewArea → Rock → StarGarden
→ Flowers → Plant → Pet → Butterfly → MysteryGate
```
...
Phase 0 的赌注是「去掉代币之后，世界变化本身能不能撑住动机」。**D1–D7 留存回答不了
这个问题** —— 7 天弧线是干预本身，留存高只说明"连载有人追"。唯一干净的信号是
**故事讲完之后、没有新东西可解锁时还回不回来**，所以 **D8 才是主验收点**。

- 导出的 `postWeek` 记录第 8 天及以后的打开情况（`daysActive` / `firstDayActive` /
  `lastDayActive` / `byDay`）。它由 `session_started` 的**时间戳**相对 `startedAt`
  推算 —— `experiment_day` 按设计被钳在 1–7（游戏里没有第 8 天，UI 不能假装有），
  所以这个信号只能在分析层算。边界是距开始的整 24 小时，不是日历日期。
- `pnpm analyze` 的「按年龄分档」表有 **D8+ 回访** 列，表下给出主验收点判读；
  阈值 30% 是**事先写在评估文档 §8 里的决策线**，直接引用在脚本常量里。
- 没有 `postWeek` 的旧导出判为**无法判读**，绝不判成"没回来"。

> ⚠️ **D8 不是一个采集开关，而是导出时机。** 标准的 7 天导出里 D8+ 一定还是 0 ——
> 那不代表孩子没回来，只代表文件取早了。实验流程必须写明：**每个参与者 Day 7 结束后
> 再过几天补导一次**。`/debug/export` 页面会在 D8+ 还是 0 时提示这一点。
...
> `reward_viewed` 的两个属性必须**一起**读：
>
> - `stages_seen`（1–4）是**机械计数**，即有几段上过屏。跳过会直接跳到第 4 段，
>   所以按"最远到达的阶段"计数会把跳过误记成全部看完（`e2e` 有专门测试守着）。
> - `skipped`（布尔）是**意图**：有没有按下「跳过」。
>
> 奖励序列现在是**轻点继续**推进的（见下面「奖励节奏」），所以一个快速连点的孩子
> 也能拿到 `stages_seen: 4` 而根本没读；只看 `skipped` 又会漏掉安静点过去的孩子。
> 判读：`stages_seen` 高 + `skipped` 低 = 在逐段看；低 + 高 = 明确想退出；
> 高 + 高 = 点了几下之后放弃。
>
> **两个都不是"看懂了吗"的证据** —— 那是 3 分钟延迟回忆探针要回答的。

### 奖励节奏（6–8 岁可读性）

四段反馈原本用固定计时器（900 / 900 / 1700 / 2500 ms）自动推进 —— 那是成人阅读
速度。第 3 段最多有约 25 个汉字，900ms 对一年级孩子不是"快"，是**看不见**；而当时
屏幕上唯一的控件是角落里的浅灰「跳过」，是全 App 最不像按钮的按钮。读得慢的孩子
没有任何办法让它慢下来。

现在：

- **轻点继续**是主要推进方式。左上角出现「轻点继续」，它只在当前段已画出来之后
  才可点（约动画时长的 35%，下限 500ms），所以点得快也不会跳过那一下的变化本身。
- **自动前进**是兜底（孩子什么都不做时才走），时长按录音的 `age_band` 缩放：
  11–12 岁 ×1.0，10–11 ×1.15，9–10 ×1.3，8–9 ×1.45，6–8 与未记录 ×1.6。
  取「未记录用最慢档」是刻意的：慢一点对熟练阅读者无害，快一点对初学者有害。
- 「跳过」变成带边框的按钮（仍然是右上角、仍然 ≥44×44），语义没变：它才是"我要走"。

### 按年龄分档（6–12 扩龄之后必看）

报告里有一节 `## 按年龄分档`，把参与者按 `age_band` 分档统计 `stages_seen`：

| 年龄组 | 参与者 | reward 观看次数 | 平均段数 | 判读 |
| --- | --- | --- | --- | --- |
| 一年级 | 2 | 42 | 1.3/4 | ⚠️ 低于 2.5 —— 这个年龄段的动画基本被跳过 |
| 六年级 | 2 | 42 | 3.8/4 | ✅ ≥ 2.5 —— 行为→世界变化这一段被看到了 |
| 未记录 | 1 | 21 | 4.0/4 | 样本不足（1 人），不能判读 |

**为什么必须分档**：上表混池均值是 2.9/4，高于 2.5 的判据 —— 只看汇总会得出
「动画被看到了」，而实际上**一年级整段都没看**。这正是扩龄要回答的问题，混池会把它
抹平。所以：

- 汇总表里 `reward 观看` 那行**故意不给 ✅/⚠️**，判读只出现在分档表
- 每档少于 2 人**只报数、不判读**（一个人的均值是轶事）
- 年龄未记录的参与者单独成行，**不能用来支持任何分龄结论**；报告会打印占比
- 无法识别的档位值（手改过文件、旧版本）一律算「未记录」，绝不会被猜成一个档
