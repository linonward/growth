...
### 世界的第三条轴：岛的样子由「做过什么」长出来

`WorldState.growthMix`（`domain/growth-mix.ts`）= 已完成目标按「学习 / 学校 / 生活」的计数，
派生自 `goalsByDay` + `GoalTemplate.group`，**不新增采集、不花星星**。渲染成
`Grove`：每 2 个学习目标 1 棵树、每 2 个生活目标 1 丛花、每 2 个学校目标 1 块小石头
（各自有上限，否则三天满勤会把岛变成森林）。

它和星星园的区别：星星园回答「**我来了**」，Grove 回答「**我做的事看得见**」。
两个分数相同的孩子因此可以看到不同的岛——这是评估文档 §4 指出"从未被测量过"的那件事。

首页世界名下面有一句归因文案（取占比最大的那一类）；**什么都不做时说的是接下来会怎样**，
不能默认成某一类，否则孩子会读到"这棵树来自你还没做的事"。抓图时把树画小了三次才对：
太小就只是噪点，读不出是树。

**它同样要接进 `store/hooks.ts` 的 `useGrowth()`。** 那里有自己的 `getGrowthState()`
调用，不走 store 的 `selectGrowth`——这个坑在星星园上犯过一次，在 Grove 上又犯了一次
（症状：所有 domain 单测全绿，屏幕上什么都没有）。`e2e` 的两条 Grove 测试就是这个守卫。
...
**`?debug=1` 的面板会挡住页面底部的按钮（已知，尚未修）。**
它 `fixed bottom-3` 且默认展开，会盖住「开始今天的成长」「创建我的世界」
这类底部主按钮。写脚本或用 Playwright 驱动时，先收起面板
（点 `debug-toggle`）或对目标按钮用 `{ force: true }`。
**这个坑今天连续踩了四次**（抓图脚本两次、e2e 一次、颜色比对一次），
每次表现都像"测试挂了"而不是"面板挡住了"。

**世界场景的所有颜色必须在 `src/components/world/palette.ts` 里。**
`tests/components/palette.test.ts` 会扫描 `src/components/world/**/*.tsx`，
出现任何 `#rgb` / `#rrggbb` 字面量就失败。原因不是洁癖：只有一种画风时，
内联颜色完全无害；一旦有第二种画风，漏掉的那块（天空渐变、地面五层、彩虹七条、
阴影）会**看起来像渲染 bug**，而不是像忘了改的变量。

**不要用"像素相同"来证明视觉重构无变化——会被动画帧骗。**
`anim-drift` / `anim-twinkle` / `anim-sway` 和太阳的闪烁会让两次截图差出几 KB，
看起来像改坏了。证明"颜色没变"要用**颜色集合等价**：把重构前的
`palette.ts` + 各精灵文件里的十六进制值取出来（记得把 `#abc` 展开成 `#aabbcc`），
和重构后取出来的集合比对，要求**零丢失、零新增**。这个方法同时还证明了
"没有主题关键的颜色残留在组件里"——因为残留会让重构后的集合多出值来。
...
已有时间戳。唯一为它加过的采集是 `reward_viewed` 的两个属性：
- `stages_seen`（1–4）：**机械计数**，即有几段上过屏。**跳过会直接跳到第 4 段**，
  所以计数必须在按下跳过时冻结；按"最远到达的阶段"计数会把跳过误记成全部看完
  （`e2e` 有测试守着）。
- `skipped`（布尔）：是否按下了「跳过」按钮。

**两者必须一起读，任何一个单独看都会误导。** 奖励序列现在是**轻点继续**推进的，
所以一个快速连点的孩子也能拿到 `stages_seen: 4` 而根本没读；反过来只看 `skipped`
又会漏掉「安静地点过去」的孩子。判读示例：`stages_seen` 高 + `skipped` 低 =
真的在逐段看；`stages_seen` 低 + `skipped` 高 = 明确想退出；
`stages_seen` 高 + `skipped` 高 = 点了几下之后放弃。

**都**不是"看懂了吗"的证据 —— 那是 3 分钟延迟回忆探针要回答的（见用研文档 §7 P3-11）。

**D8 是主验收点，而它是一个「导出时机」问题，不是一个采集开关。**
`export.postWeek` 记录第 8 天及以后的打开情况，**由 `session_started` 的时间戳相对
`startedAt` 推算** —— `experiment_day` 按设计被钳在 1–7（游戏里没有第 8 天，UI 不能
假装有），所以这个信号只能在分析层算。边界是距开始的**整 24 小时**，不是日历日期：
孩子几点睡不该移动这个指标。`pnpm analyze` 的「按年龄分档」表有 **D8+ 回访**列，
阈值 `D8_RETURN_FLOOR = 30%` **直接引用评估文档 §8 事先写下的决策线**，
不许看完数据再改。没有 `postWeek` 的旧导出判为**无法判读**，绝不判成"没回来"。

> **标准 7 天导出里 D8+ 一定是 0，那不代表孩子没回来，只代表文件取早了。**
> 实验流程必须写明：每个参与者 Day 7 结束后**再过几天补导一次**。
> `/debug/export` 页面会在 D8+ 还是 0 时提示这一点。
